...
local function structurize (raw,prefix)
  prefix = prefix or ''
  ...
  for column,value in pairs(raw) do
    ...
    if name then
      ...
      if related then
        row[related] = structurize(raw,prefix..related..'_')
      else
        row[name] = value
      end
    end
...
local DBI = {
  ...
  offset = function (self,n)
    self.params.offset = n
    return self
  end,
  
  get = function (self, row)
    ...
    for _,ROW in pairs(self.driver.fetch()) do
      table.insert(ROWs,structurize(ROW)[self.name])
    end
    return ROWs
  end
}
